...
- **Integration tests exist** in `internal/client/integration_test.go` and `internal/cleanup/integration_test.go` (build tag `integration`). They test against a real TrueNAS via WebSocket.
