...
### Contributors
- **Emil Larsson** ([@emil-jacero](https://github.com/emil-jacero)) — reported and fixed the `wsTransport.Close` WaitGroup-reuse panic (see *Reliability* below). Root-caused it in production, wrote the fix and regression test on a downstream fork ([`emil-jacero/omni-infra-provider-truenas@b7d9475`](https://github.com/emil-jacero/omni-infra-provider-truenas/commit/b7d9475e40250627e21abc56a2fa866520ca745a)), and shared it back through the fork's public history even though this project runs an issues-only contribution model. Cherry-picked with Emil preserved as the git author. Thank you.

### Reliability
- **WebSocket `Close()` no longer races with in-flight `Call`/`UploadFile`.** `Close` flipped `t.closed = true` only *after* `wg.Wait()` returned, so a concurrent `Call` or `UploadFile` could `wg.Add(1)` at the exact moment `Wait`'s counter hit zero — the interleaving `sync.WaitGroup` forbids, panicking the provider with `sync: WaitGroup is reused before previous Wait has returned` in a loop shortly after reconnects. Fix flips `closed` under `connMu`'s write lock *before* `Wait`, and gates `wg.Add(1)` in `Call`/`UploadFile` on the same flag under the read lock so an in-flight `Add` is guaranteed to land before `Close`'s `Wait` can start. `Close` is idempotent now (repeated/concurrent calls no-op after the first). Ships with `TestWS_CloseDoesNotRaceWithConcurrentCalls`. See *Contributors* above for attribution.
